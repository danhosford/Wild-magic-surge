...
IF OBJECT_ID('test.create_MOC_Form') IS NULL
    EXEC ('CREATE PROCEDURE test.create_MOC_Form AS SELECT 1');
GO

CREATE OR ALTER PROCEDURE test.create_MOC_Form
(
@name VARCHAR(255)
,@description VARCHAR(255)
,@isApproverByLocation BIT
,@ApproverLevel INT
,@moctype INT
...
BEGIN
	...
	DECLARE @MODULE_PREFIX VARCHAR(255) = 'moc';
	...
	,ksa.kioskSiteUUID
	-- Form params
	,'','/SafePermitApp/moc/mocCreateSubmit/mocCustomCreateSubmit_after.cfm'
	...
	INSERT INTO mocFormTypeSetting([formTypeID]
	,[mocFormTypeSettingIsActive]
	,[mocFormTypeSettingApprovalRequired]
	,[mocFormTypeSettingCloseRequestRequired]
	,[mocFormTypeSettingAddTasksAfterApproval]
	,[mocFormTypeSettingApproverAddActionTask]
	,[mocFormTypeSettingCanModifyTaskOwners]
	,[mocFormTypeSettingCreateBy],[mocFormTypeSettingCreateUTC]
	,[kioskID]
	,[kioskSiteUUID]
	,[mocFormTypeSettingMocType]
	,[mocFormTypeSettingMocUsesAcl]
	,[mocFormTypeSettingAttachBusinessCase]
	,[mocFormTypeSettingApproverByLocation]
	,[mocFormTypeSettingApproverByLocationLevel])
	SELECT 
	bcf.id,1,1,0,0,0,0
	,0,GETUTCDATE()
	,@KIOSKID,bcf.kioskSiteUUID
	,@moctype,0,0,@isApproverByLocation,@ApproverLevel
	FROM #BC_FORM AS bcf
	LEFT JOIN mocFormTypeSetting AS mfts ON mfts.kioskID = @KIOSKID
		AND mfts.kioskSiteUUID = bcf.kioskSiteUUID
		AND mfts.formTypeID = bcf.id
	WHERE mfts.mocFormTypeSettingID IS NULL;
