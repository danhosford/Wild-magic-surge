...
IF OBJECT_ID('test.create_CP_Form') IS NULL
    EXEC ('CREATE PROCEDURE test.create_CP_Form AS SELECT 1');
GO

CREATE OR ALTER PROCEDURE test.create_CP_Form
(
@name VARCHAR(255)
...
AS
BEGIN
	DECLARE @MODULE_PREFIX VARCHAR(255) = 'cp';
	...
	INSERT INTO [dbo].[cpReferenceCompanyComplianceRequirement]
		([kioskID],[kioskSiteUUID]
		,[cpReferenceCompanyComplianceRequirementName]
		,[cpReferenceCompanyComplianceRequirementIsActive]
	...
	SELECT ft.kioskID, ft.kioskSiteUUID
	,CONCAT(ft.formName,' - ',ks.kioskSiteName), 1,
	0,1
	,ft.formTypeID, 0	 
	FROM dbo.formType AS ft
	LEFT JOIN dbo.cpReferenceCompanyComplianceRequirement AS rccr ON rccr.formTypeID = ft.formTypeID
		AND rccr.kioskID = ft.kioskID
		AND rccr.kioskSiteUUID = ft.kioskSiteUUID
		AND rccr.cpReferenceCompanyComplianceRequirementName = ft.formName
	LEFT JOIN dbo.kioskSite AS ks ON ks.kioskSiteUUID = ft.kioskSiteUUID
	WHERE ft.formName = @name
		AND rccr.cpReferenceCompanyComplianceRequirementID IS NULL;
