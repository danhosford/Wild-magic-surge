...
-- Description: Generate active permit
-- CHANGELOG:
...
DECLARE @TOTAL_PERMIT INT = 25;
DECLARE @NUMBER_MAX_DAY INT = 30; 
...
SELECT @CURRENTCOUNT = COUNT([pcID]) FROM [permitCreateNumber];
...
BEGIN
  ...
  BEGIN
    ...
    DECLARE @CREATION_DATE DATETIME = DATEADD(DAY, -ABS(CHECKSUM(NEWID()) % (@NUMBER_MAX_DAY)), GETUTCDATE());
    ...
    DECLARE @START_TIME DATETIME = DATEADD(DAY, RAND(CHECKSUM(NEWID())) * (1 + DATEDIFF(DAY,@CREATION_DATE,GETUTCDATE())),@CREATION_DATE)
    SET @START_TIME = test.RoundTime(DATEADD(HOUR,ROUND((23 * RAND() + 1),0),@START_TIME),0.25);
    DECLARE @END_TIME DATETIME = test.RoundTime(DATEADD(DAY,ROUND(((23 -2) * RAND() + 1), 0),GETUTCDATE()),0.25);
    DECLARE @FORMCREATEID INT = (SELECT TOP (1) [pcID] FROM [dbo].[permitCreate] ORDER BY pcID DESC);
    DECLARE @PERMIT_NUMBER VARCHAR(255) = RIGHT('000'+CAST( ROUND(((@Upper - @Lower -1) * RAND() + @Lower), 0) AS VARCHAR(3)),3);
    ...
    ,CONCAT(UPPER(FORMAT(@START_TIME, 'MMMyy')),'-AUTO',LEFT(ks.kioskSiteName,2),@PERMIT_NUMBER),pt.ptID
    ,'Auto generated - Active purpose','Auto generated - Active purpose'
    ,NEWID(),NEWID()
    ,1,0
