...
BEGIN
    ...
    INSERT INTO permitType ( [kioskID], [kioskSiteUUID]
                           ...
                           , [ptUseACL]
                           , [ptConflictManagerLocationLevel1FieldName], [ptConflictManagerLocationLevel2FieldName]
                           , [ptConflictManagerLocationLevel3FieldName]
                           , [ptConflictManagerLocationLevel3AdditionalFieldName]
                           ...
                           , [ptMaxExtensionDays], [ptMaxExtensions], [ptCanCloserAddSignature], [ptCanChangeApprover]
                           , [ptCanChangePreApprover]
                           , [ptCanBeSuspended]
                           , [ptCanBeSuspendedBy]
                           , [ptCanBeRenewedBy])
    SELECT DISTINCT @KIOSKID
                  , ks.kioskSiteUUID
                  , 0
                  , GETUTCDATE()
                  , pt.[name]
                  , pt.[name]
                  , pt.[ptIsActive]
                  , pt.[ptIsGeneralPermit]
                  , pt.[ptInitial]
                  , pt.[ptIsHazardous]
                  , pt.[ptIsHotWorkPermit]
                  , pt.[ptIsLockoutPermit]
                  , pt.[ptPrintSetupFile]
                  , pt.[ptLogoImage]
                  , pt.[ptIsDocumentAttachMandatory]
                  , pt.[ptCanSelfApprove]
                  , pt.[ptIsContractorSearchMandatory]
                  , pt.[ptIsDocumentSearchMandatory]
                  , pt.[ptOrder]
                  , pt.[ptIsPermitConflict]
                  , pt.[ptIsConflictManager]
                  , pt.[ptDisplayRequirementBar]
                  , pt.[ptUseACL]
                  , pt.[ptConflictManagerLocationLevel1FieldName]
                  , pt.[ptConflictManagerLocationLevel2FieldName]
                  , pt.[ptConflictManagerLocationLevel3FieldName]
                  , pt.[ptConflictManagerLocationLevel3AdditionalFieldName]
                  , pt.[ptConflictManagerStartDateOfWorkFieldName]
                  , pt.[ptConflictManagerEndDateOfWorkFieldName]
                  , pt.[ptConflictManagerStartTimeFieldName]
                  , pt.[ptConflictManagerEndTimeFieldName]
                  , pt.[ptMandatoryToClosePermit]
                  , pt.[isTimeValidationStartBeforeEndTimeMandatory]
                  , pt.[isTimeValidationCreateInPastNotAllowed]
                  , pt.[maxLengthOfPermitDay]
                  , pt.[maxLengthOfPermitHour]
                  , pt.[ptColour]
                  , pt.[ptContractorModuleInUse]
                  , pt.[ptApproverByLocation]
                  , pt.[ptApproverByLocationLevel]
                  , pt.[ptValidateWorkforce]
                  , pt.[ptCanAddWorkforce]
                  , pt.[ptCanAddSignature]
                  , pt.[ptValidateCompany]
                  , pt.[ptCanApproverAddSignature]
                  , pt.[ptMaxExtensionDays]
                  , pt.[ptMaxExtensions]
                  , pt.[ptCanCloserAddSignature]
                  , pt.[ptCanChangeApprover]
                  , pt.[ptCanChangePreApprover]
                  , pt.[ptCanBeSuspended]
    ...
    FROM [permitType] AS [permit]
             INNER JOIN @FormFields AS [fields] ON [fields].[name] = [permit].[name]
    WHERE ([permit].[ptIsActive] != [fields].[ptIsActive]
       ...
       OR ([permit].[ptConflictManagerLocationLevel3AdditionalFieldName] !=
           [fields].[ptConflictManagerLocationLevel3AdditionalFieldName]
       ...
       OR ([permit].[isTimeValidationStartBeforeEndTimeMandatory] !=
           [fields].[isTimeValidationStartBeforeEndTimeMandatory]
    ...
    INSERT INTO [permitPage]( [kioskID], [kioskSiteUUID]
                            , [ptID], [ppName], [ppOrder]
                            , [ppIsActive]
                            , [ppCreateBy], [ppCreateUTC])
    SELECT DISTINCT pt.kioskID
                  , pt.kioskSiteUUID
                  , pt.ptID
                  , pf.ppName
                  , pf.ppOrder
    ...
    INSERT INTO [permitField]( [kioskID], [kioskSiteUUID], [ptID]
    ...
    INSERT INTO [dbo].[permitWorkflow]( [kioskID], [kioskSiteUUID]
    ...
    SELECT DISTINCT pt.kioskID
                  , pt.kioskSiteUUID
                  , pt.ptID
                  , statusin.[permitStatusID]
                  , statusout.[permitStatusID]
                  , 1
                  , pw.[order]
                  , 0
                  , GETUTCDATE()
    ...
    INSERT INTO [dbo].[permitDropDownLocation]( [kioskID], [kioskSiteUUID], [pddlIsActive]
                                              , [pfID], [klID])
    SELECT DISTINCT pt.kioskID
                  , pt.kioskSiteUUID
                  , 1
    ...
    INSERT INTO [dbo].[permitTypeACL]( [kioskID], [kioskSiteUUID]
...
INSERT INTO permitRequirement ( [kioskID]
                              , [pfID], [prRequirement]
                              , [prType], [pfFieldValue], [ptID], [prIsActive], [kuCreateBy]
                              , [kioskSiteUUID])
SELECT @kioskID,
       pf.pfID,
       cqt.cqtID,
       ...
       '0',
       site.kioskSiteUUID
FROM permitField as pf
         INNER JOIN permitType as pt
                    ON pt.ptID = pf.ptID
                        AND pt.ptName = 'General with requirements'
         INNER JOIN kioskSite as site
                    ON site.kioskID = @kioskID
         INNER JOIN contractorQualificationType as cqt
                    ON cqt.kioskID = @kioskID
WHERE pf.pfNarrative in ('Location of Work', 'Description of Work')
  AND cqtName IN ('Site Induction', 'CSR Card')
GO